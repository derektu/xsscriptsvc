...
class API {
    ...
    /**
     * http://<server>/api/zipuserscripts?appid=<AppID>&userid=<UserID>&type=<ScriptType>&keepfolder=1
     * 
    ...
    async api_ZipUserScripts(req, res) {
        try {
            ...
            let zipOption = {
                userPrefix: false,
                keepFolder: req.query.keepfolder === undefined ? false : true
            }
            await new XSScriptBundler().bundleUserScripts(this.xsservice, appId, userId, scriptType, zipFullpath, zipOption);
        ...
        upload(req, res, (err)=> {
            try {
                ...
                let csvOption = new CSVOption(
                    req.body['has_headerrow'] === undefined ? false : true,
                ...
                let zipOption = {
                    userPrefix: true,
                    keepFolder: req.body['keeyfolder'] == "1"
                }

                this.procZipFromCSVTask(req, timestamp, csvFilename, csvOption, zipFullPathname, zipOption)
                    .then((data)=> {
                        res.json(data);
                    })
                    .catch((err)=> {
     ...
     * @param {*} zipOption {userPrefix, keepFolder}
     */
    async procZipFromCSVTask(req, timestamp, csvFilename, csvOption, zipFullPathname, zipOption) {
     ...
     * @param {string} taskId
     * @param {*} task { csvFilename, csvOption, zipFullPathname, zipUrl, zipOption} 
     */
    async zipProc(taskId, task, updateProgress) {
        let {csvFilename, csvOption, zipFullPathname, zipUrl, zipOption} = task;

        logger.debug(`zipProc(${taskId}) started`);

        await new XSScriptBundler().bundleScriptsFromCSV(this.xsservice, csvFilename, csvOption, zipFullPathname, zipOption, updateProgress);
