...
const {CSVOption, XSScriptZipGenerator} = require('./xsscriptzipgenerator');
...
class API {
    ...
    getRouter() {
        ...
        router.get('/sensorscript', (req, res)=> this.api_GetSensorScript(req, res));
     ...
     * 
     * 傳入指定的sensor, 回傳sensor所對應的腳本object
     * Note: 如果sensor跑的是系統腳本, 則return {}
     * 
     * @param {*} req 
     * @param {*} res 
     */
    async api_GetSensorScript(req, res) {
        try {
            let appId = req.query.appid;
            if (!appId)
                throw `Missing argument: appId`;
            ...
            let sensors = await this.xsservice.querySensors(appId, userId, guid);
            if (sensors.length == 0 || sensors[0].groupId != "") {
                res.json({});
            }
            else {
                let script = await this.xsservice.queryScriptById(appId, userId, XSScriptType.Sensor, sensors[0].scriptId);
                if (!script) {
                    res.json({});
                }
                else {
    ...
    async api_ZipUserScripts(req, res) {
        try {
            ...
            await new XSScriptZipGenerator().zipUserScripts(this.xsservice, appId, userId, scriptType, zipFullpath, false);
    ...
    async api_ZipScriptsFromCSV(req, res) {
        /*
            ...
            col_type: scripttype是那個column (0-based), 預設=2
            col_guid: guid是那個column (0-based), 預設=3
        */
        ...
        upload(req, res, (err)=> {
            try {
                ...
                let csvOption = new CSVOption(
                    ...
                    parseInt(req.body['col_type'] || '2'),
                    parseInt(req.body['col_guid'] || '3')
                );
    ...
    async zipProc(taskId, task, updateProgress) {
        ...
        await new XSScriptZipGenerator().zipScriptsFromCSV(this.xsservice, csvFilename, csvOption, zipFullPathname, true, updateProgress);
