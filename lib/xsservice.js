...
class XSService {
     ...
     * @param {string} guid 
     * @returns {*} return XSScript object. Exception will be thrown if any error  
     */
     ...
     * @returns {*} array of XSScript. Exception will be thrown if error.
     */
     ...
     * @param {*} guids Guid to lookup. 可以傳入一個array of strings, or string(';'-separated)
     * @returns {*} return array of {guid, groupId, scriptId}. groupId/scriptId是對應的腳本. 如果查不到的話, 就回空的.
     */
    async querySensors(appId, userId, guids) {
        if (!Array.isArray(guids))
            guids = guids.split(";");

        let xml = this._composeSensorQueryXml(appId, userId, guids);
    ...
    _composeSensorQueryXml(appId, userId, guids) {
        /*
         <?xml version='1.0' encoding='utf-8' ?>
    ...
    _parseSensorQueryXml(doc) {
        ...
        */
        ...
        for (let i = 0; i < sensorNodes.length; i++) {
            ...
            let guid = node.getAttribute("SID");
            ...
            try {
                ...
                //
                sensors.push({guid: guid, groupId: groupId, scriptId:scriptId});    
            }
            catch(exception) {
                logger.error(`_parseSensorQueryXml exception:${exception}`);
            }
        }    
        return sensors;
    }
