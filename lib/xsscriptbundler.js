...
class XSScriptBundler {
    ...
    async bundleScriptsFromCSV(xsservice, csvFilename, csvOption, target, bundleOption, cbProgress) {
        ...
        // 因為sensorrun.csv內可能會有多筆sensor對到同一個腳本的case, 所以這裡設定一個map, 
        // key = AppID-UserID, value = Set of guid, 確保guid不會重複
        // - 不需要管scriptType, 因為不同的scriptType的guid也不會重複
        // - 腳本如果匯出的話, 會維持同一個guid, 可是會放在另一個人的身上, 這個我們就照常放進zip內
        //
        let guidMap = {};
        ...
        for (let line of lines) {
            ...
            try {
                ...
                // check if this guid is unique for this user
                //
                let key = `${appId}-${userId}`;
                let guidSet = guidMap[key] || new Set();
                if (guidSet.has(guid)) {
                    logger.debug(`[${basename}](LINE:${lineno}) ignore guid:${guid}`);
                    continue;
                }
